...
dados_cliente = pd.read_csv(
    "dados/brutos/cliente.csv",
    dtype={"cpf": str},
)
dados_analise_credito = pd.read_csv("dados/brutos/analise_credito.csv")
...
def transformacao_tabela_analise_credito(dados_analise_credito, dados_cliente_tratados):
    ...
    # Padroniza texto
    dados_analise_credito["status_pagamento"] = dados_analise_credito["status_pagamento"].str.lower().str.strip()
    dados_analise_credito["flag_fraude"] = dados_analise_credito["flag_fraude"].str.lower().str.strip()

    # Alterando texto das colunas
    dados_analise_credito["status_pagamento"] = dados_analise_credito["status_pagamento"].map({"em dia":1, "atrasado":2, "inadimplente": 3})
    dados_analise_credito["flag_fraude"] = dados_analise_credito["flag_fraude"].map({"nao":0, "sim":1})
    ...
    print(dados_analise_credito.isnull().sum())
...
arquivo_dados_cliente_tratados = "dados/tratados/dados_cliente_tratados.csv"
...
arquivo_dados_analise_credito_tratados = "dados/tratados/dados_analise_credito_tratados.csv"
